# Composants tiers

Le code ORION est sous AGPL-3.0-only. Les dépendances conservent leurs licences respectives ; les fichiers exacts se trouvent dans les paquets installés, et leurs versions/intégrités dans `package-lock.json`.

- React et React DOM — MIT.
- Lucide React (icônes) — ISC.
- Zod — MIT.
- fflate (archives ZIP des documents bureautiques) — MIT.
- jsPDF et jsPDF-AutoTable (PDF) — MIT.
- Vite, TypeScript, Prettier et leurs dépendances — licences propres aux paquets, outils de développement uniquement.
- IBM Plex Sans / Mono — SIL Open Font License ; textes dans `docs/licenses/`. Les polices sont hébergées localement.

Les anciens signes OFPP et tuiles swisstopo ne sont plus distribués par le journal 1.0. Les références métier citées ne sont pas une approbation du produit.
